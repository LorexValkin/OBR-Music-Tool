...
use format::{FormatError, RawIndex, EV_HAS_SHARED_MEDIA, EV_LOCALISED, EV_PLUGIN, EV_PREFETCH_SUSPECT, NONE, WEM_LOCALISED, WEM_PLUGIN, WEM_SHARED};
use std::ops::Range;
...
impl TabKind {
    ...
    /// Tabs whose contents are not shipped yet are shown disabled (none today).
...
impl Event {
    ...
    /// All of this sound's media are generated by a Wwise plugin: nothing to replace.
    pub fn plugin(&self) -> bool {
        self.flags & EV_PLUGIN != 0
    }
...
pub struct Wem {
    ...
    pub shared: bool,
    /// Plugin-generated media (`PLUG` file), not a replaceable audio file.
    pub plugin: bool,
...
mod tests {
    ...
    fn embedded_index_loads_and_is_consistent() {
        ...
        for tab in idx.tabs() {
            // Dialogue rows come from the voice index, not from this one.
            if tab.kind != TabKind::Dialogue {
