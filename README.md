...
- **Lists every vanilla music track, sound effect and voice line** in tabs: Music, Menu & UI, Weapons, Magic,
  ...
  `al_obj_drs_chest_open.wav`), so you are never guessing from numeric ids. The Dialogue tab groups every voice
  line by speaker: open *Sheogorath* and all 320 of his lines are there, with the subtitle, the voice type and the
  play length. Tabs show 50 sounds per page; the search box covers the whole tab.
...
3. **Choose your sounds.** Pick a tab at the top of the inventory (Music, Menu & UI, Weapons, ...) and use the
   ...
   **Remove** un-queues one and **Clear all** resets everything. Replacement audio can be any length, sample rate
   ...
   you replace one of them, and the other sounds then show as *Partial* or *Edited* too. A few weapon-impact
   ...
   file and cannot be replaced. Every sound tab starts with a pinned **Shared audio** group: the audio files used by
   several sounds (one menu click serves 90 actions), each listing the sounds that use it, so the shared files can
   be edited once in one place.

   In the **Dialogue** tab each row is a speaker: a named NPC, a faction or class, or *Any Orc female* for the
   lines every NPC of that race and sex can say. Click the arrow (or double-click) to open the speaker's lines: each
   shows its topic, the subtitle (hover it to read the whole line), the voice type and its length, with its own
   Play, Replace and Remove. Search matches speakers, words in the lines, topics, quests and voice types, so
   `haskill`, `sheogorath greeting` or `orc female rumors` work; the speaker whose name matches is listed first and
   only the matching lines are shown under each speaker. The first entry, **Generic lines**, holds the lines every
   NPC of a race can say (greetings, rumors, guard barks...): it opens into one row per race, then per sex, then the
   lines. It is flagged on purpose - replacing a line there changes it for a mass of characters. The game shares recordings between races (Dark Elf and
   Wood Elf lines use the High Elf actor, Orc uses Nord, Khajiit uses Argonian, Breton uses Imperial): such a
   line says *same recording as …*, and replacing it changes it for every race that plays it.
...
1. Every sound (28 music tracks and ~1,200 effects, ~5,700 audio files) is mapped to the numeric Wwise media
   ...
   (`assets/voice_index.bin`, stored compressed, decoded once on first use) maps the 135,046 voice lines (94,284
   distinct recordings) to their dialogue records: the voice events name the recording, the game's plugins (`Oblivion.esm` and the DLC `.esp` files
...
| Dialogue | Every voice line grouped by speaker (NPC, faction, class or race), with subtitle, topic and voice type | 1,127 speakers, 135,046 lines |
